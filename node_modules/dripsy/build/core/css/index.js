...
import { get } from './get';
import { aliases, scales } from './scales';
const defaultTheme = {
    space: {
        $0: 0,
        $1: 4,
...
const responsive = (styles, { breakpoint } = {}) => (theme) => {
    const next = {};
    for (const styleKey in styles) {
        const value = typeof styles[styleKey] === 'function'
        ...
        const nearestBreakpoint = (breakpointIndex) => {
 ...
 */
const filterWebStyleKeys = (styleProp = {}) => {
    if (Platform.OS == 'web') {
        return styleProp;
    }
...
export const css = (args = {}, breakpoint
// { ssr }: { ssr?: boolean } = {}
) => ({ themeKey, fontFamily: fontFamilyFromProps, ...props } = {}) => {
    ...
    for (const key in styles) {
        ...
        if (key == 'variant') {
            ...
            const variant = css(get(theme, themeKey + '.' + val, get(theme, val)), breakpoint)({ theme });
        ...
        if (val === '') {
            console.error(`[dripsy] Invalid style. You passed an empty string ('') for ${key}. Please fix this.`);
            continue;
        }
        ...
        if (key == 'fontWeight' && styles?.fontWeight) {
            ...
            const fontFamilyKeyFromStyles = styles?.fontFamily ?? fontFamilyFromProps;
            ...
            if (fontFamilyKeyFromStyles || rootFontFamilyFromTheme) {
                ...
                if (fontFamily) {
                    ...
                    const customFontFamilyForWeight = theme?.customFonts?.[fontFamily]?.[fontWeight];
